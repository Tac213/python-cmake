...
if(WIN32)
    ...
    if(MSVC)
        if(${CMAKE_SYSTEM_PROCESSOR} MATCHES "(AMD64)|(amd64)")
            set(PEP11_TARGET "x86_64-pc-windows-msvc")
        elseif(${CMAKE_SYSTEM_PROCESSOR} MATCHES "x86")
            set(PEP11_TARGET "i686-pc-windows-msvc")
        endif()
    endif()
elseif(APPLE)
...
if(WIN32)
    ...
    string(REGEX MATCH "openssl-bin-([0-9]+\.)+([0-9]+)u?" _win32_openssl_folder_name ${_win32_get_externals})
...
# Not available on win32
    ...
    if(PY_VERSION VERSION_GREATER_EQUAL "3.11")
        if(GDBM_COMPAT_LIBRARY)
            check_include_files(gdbm/ndbm.h HAVE_GDBM_NDBM_H)

            if(NOT HAVE_GDBM_NDBM_H)
                check_include_files(gdbm-ndbm.h HAVE_GDBM_DASH_NDBM_H)

                if(NOT HAVE_GDBM_DASH_NDBM_H)
                    if(NDBM_INCLUDE_PATH)
                        set(NDBM_TAG NDBM)
                    endif()
                else()
                    set(NDBM_TAG GDBM_COMPAT)
                endif()
            else()
                set(NDBM_TAG GDBM_COMPAT)
            endif()
            ...
            if(HAVE_DB_H)
                set(NDBM_TAG BERKDB)
                find_library(BERKDB_LIBRARY NAMES db-5.3)
            endif()
        endif()
    else()
        if(NDBM_INCLUDE_PATH)
...
int bar(void) {};
int main() {bar();}" HAVE_HIDDEN_VISIBILITY_ATTRIBUTE)
...
int main(void){
  ...
  uuid_generate_time_safe(out);
        ...
        set(CFG_HEADERS ${CFG_HEADERS_SAVE})
        cmake_pop_check_state()
    endif() # if(PY_VERSION VERSION_GREATER_EQUAL "3.7")
...
")
            ...
            endif() # if(HAVE_GETHOSTBYNAME_R_5_ARG)
        endif() # if(HAVE_GETHOSTBYNAME_R_6_ARG)
    else() # if(HAVE_GETHOSTBYNAME_R)
        check_symbol_exists(gethostbyname "${CFG_HEADERS}" HAVE_GETHOSTBYNAME)
    endif() # if(HAVE_GETHOSTBYNAME_R)

...
function(_apply_patches _subdir)
    ...
    foreach(patch IN LISTS _patches)
        ...
        string(REGEX MATCH "\\[([^()]*)\\]" _ ${patch})
        set(_need_option ${CMAKE_MATCH_1})
        if(_need_option AND (NOT ${_need_option}))
            message(STATUS "${msg} - skipping (not set: ${_need_option})")
            continue()
        endif()
        message(STATUS "${msg}")
...
if(MSVC)
    ...
    if ("${MSVC_VERSION}" VERSION_LESS "2000" AND ("${MSVC_VERSION}" VERSION_GREATER_EQUAL "1900"))
        set(_version "1900")
    else()
        set(_version ${MSVC_VERSION})
    endif()
...
if(EXISTS "${SRC_DIR}/.git")
    file(REMOVE_RECURSE "${SRC_DIR}/.git")
endif()
